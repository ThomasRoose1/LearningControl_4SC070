/*
 * SPERTE_printer_RC.h
 *
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * Code generation for model "SPERTE_printer_RC".
 *
 * Model version              : 13.1
 * Simulink Coder version : 25.2 (R2025b) 28-Jul-2025
 * C source code generated on : Wed May 13 12:04:26 2026
 *
 * Target selection: ert.tlc
 * Note: GRT includes extra infrastructure and instrumentation for prototyping
 * Embedded hardware selection: ARM Compatible->ARM Cortex-A (64-bit)
 * Code generation objectives: Unspecified
 * Validation result: Not run
 */

#ifndef SPERTE_printer_RC_h_
#define SPERTE_printer_RC_h_
#ifndef SPERTE_printer_RC_COMMON_INCLUDES_
#define SPERTE_printer_RC_COMMON_INCLUDES_
#include <stdio.h>
#include "rtwtypes.h"
#include "simstruc.h"
#include "fixedpoint.h"
#include "rtw_extmode.h"
#include "sysran_types.h"
#include "rtw_continuous.h"
#include "rtw_solver.h"
#include "ext_mode.h"
#endif                                 /* SPERTE_printer_RC_COMMON_INCLUDES_ */

#include "SPERTE_printer_RC_types.h"
#include <stddef.h>
#include <string.h>
#include "rt_zcfcn.h"
#include "rt_nonfinite.h"
#include "rtGetNaN.h"
#include "zero_crossing_types.h"

/* Macros for accessing real-time model data structure */
#ifndef rtmGetContTimeOutputInconsistentWithStateAtMajorStepFlag
#define rtmGetContTimeOutputInconsistentWithStateAtMajorStepFlag(rtm) ((rtm)->CTOutputIncnstWithState)
#endif

#ifndef rtmSetContTimeOutputInconsistentWithStateAtMajorStepFlag
#define rtmSetContTimeOutputInconsistentWithStateAtMajorStepFlag(rtm, val) ((rtm)->CTOutputIncnstWithState = (val))
#endif

#ifndef rtmGetDerivCacheNeedsReset
#define rtmGetDerivCacheNeedsReset(rtm) ((rtm)->derivCacheNeedsReset)
#endif

#ifndef rtmSetDerivCacheNeedsReset
#define rtmSetDerivCacheNeedsReset(rtm, val) ((rtm)->derivCacheNeedsReset = (val))
#endif

#ifndef rtmGetFinalTime
#define rtmGetFinalTime(rtm)           ((rtm)->Timing.tFinal)
#endif

#ifndef rtmGetRTWExtModeInfo
#define rtmGetRTWExtModeInfo(rtm)      ((rtm)->extModeInfo)
#endif

#ifndef rtmGetSampleHitArray
#define rtmGetSampleHitArray(rtm)      ((rtm)->Timing.sampleHitArray)
#endif

#ifndef rtmGetStepSize
#define rtmGetStepSize(rtm)            ((rtm)->Timing.stepSize)
#endif

#ifndef rtmGetZCCacheNeedsReset
#define rtmGetZCCacheNeedsReset(rtm)   ((rtm)->zCCacheNeedsReset)
#endif

#ifndef rtmSetZCCacheNeedsReset
#define rtmSetZCCacheNeedsReset(rtm, val) ((rtm)->zCCacheNeedsReset = (val))
#endif

#ifndef rtmGet_TimeOfLastOutput
#define rtmGet_TimeOfLastOutput(rtm)   ((rtm)->Timing.timeOfLastOutput)
#endif

#ifndef rtmGetErrorStatus
#define rtmGetErrorStatus(rtm)         ((rtm)->errorStatus)
#endif

#ifndef rtmSetErrorStatus
#define rtmSetErrorStatus(rtm, val)    ((rtm)->errorStatus = (val))
#endif

#ifndef rtmGetStopRequested
#define rtmGetStopRequested(rtm)       ((rtm)->Timing.stopRequestedFlag)
#endif

#ifndef rtmSetStopRequested
#define rtmSetStopRequested(rtm, val)  ((rtm)->Timing.stopRequestedFlag = (val))
#endif

#ifndef rtmGetStopRequestedPtr
#define rtmGetStopRequestedPtr(rtm)    (&((rtm)->Timing.stopRequestedFlag))
#endif

#ifndef rtmGetT
#define rtmGetT(rtm)                   (rtmGetTPtr((rtm))[0])
#endif

#ifndef rtmGetTFinal
#define rtmGetTFinal(rtm)              ((rtm)->Timing.tFinal)
#endif

#ifndef rtmGetTPtr
#define rtmGetTPtr(rtm)                ((rtm)->Timing.t)
#endif

#ifndef rtmGetTStart
#define rtmGetTStart(rtm)              ((rtm)->Timing.tStart)
#endif

#ifndef rtmGetTimeOfLastOutput
#define rtmGetTimeOfLastOutput(rtm)    ((rtm)->Timing.timeOfLastOutput)
#endif

/* Block signals (default storage) */
typedef struct {
  real_T SFunction;                    /* '<S11>/S-Function' */
  real_T ec_Ebox_o1[2];                /* '<S10>/ec_Ebox' */
  real_T ec_Ebox_o2[2];                /* '<S10>/ec_Ebox' */
  real_T ec_Ebox_o3[8];                /* '<S10>/ec_Ebox' */
  real_T Gain[2];                      /* '<S10>/Gain' */
  real_T CountstoMeter1;               /* '<S8>/Counts to Meter1' */
  real_T RateTransition5;              /* '<S8>/Rate Transition5' */
  real_T Gain_c;                       /* '<S3>/Gain' */
  real_T Dctpd;                        /* '<S3>/Dctpd' */
  real_T Dct2lowpass;                  /* '<S3>/Dct2lowpass' */
  real_T Gain1;                        /* '<S16>/Gain1' */
  real_T Dctpd2;                       /* '<S16>/Dctpd2' */
  real_T Dct2lowpass3;                 /* '<S16>/Dct2lowpass3' */
  real_T CastToDouble;                 /* '<S16>/Cast To Double' */
  real_T Dctintegrator1;               /* '<S16>/Dctintegrator1' */
  real_T Gain1_j;                      /* '<S15>/Gain1' */
  real_T Dctpd2_k;                     /* '<S15>/Dctpd2' */
  real_T Dct2lowpass3_o;               /* '<S15>/Dct2lowpass3' */
  real_T Saturation[2];                /* '<S10>/Saturation' */
  real_T Clock;                        /* '<Root>/Clock' */
  real_T Internal;                     /* '<S23>/Internal' */
  real_T DirectLookupTableref1;        /* '<S4>/Direct Lookup Table ref1' */
  real_T Sum;                          /* '<S4>/Sum' */
  real_T jogSpeed;                     /* '<S9>/Supervisor' */
  real_T toPoint;                      /* '<S9>/Supervisor' */
  real_T homeControllerEnabled;        /* '<S9>/Supervisor' */
  real_T resetEnc;                     /* '<S9>/Supervisor' */
  real_T homingBusy;                   /* '<S9>/Supervisor' */
  real_T airBagEnabled;                /* '<S9>/Supervisor' */
  real_T airBagRef;                    /* '<S9>/Supervisor' */
  real_T controlEnabled;               /* '<S9>/Supervisor' */
  SupervisorModeType SupervisorMode;   /* '<S9>/Supervisor' */
  uint16_T Product;                    /* '<S4>/Product' */
  uint8_T status;                      /* '<S6>/SPERTE_measurement_function' */
  boolean_T UnitDelay1;                /* '<S9>/Unit Delay1' */
  boolean_T LogicalOperator2;          /* '<S9>/Logical Operator2' */
} B_SPERTE_printer_RC_T;

/* Block states (default storage) for system '<Root>' */
typedef struct {
  real_T DiscreteTimeIntegrator_DSTATE;/* '<S9>/Discrete-Time Integrator' */
  real_T UD_DSTATE;                    /* '<S18>/UD' */
  real_T Delay_DSTATE[3];              /* '<S5>/Delay' */
  real_T Delay1_DSTATE[1755];          /* '<S5>/Delay1' */
  real_T Internal_DSTATE[100];         /* '<S22>/Internal' */
  real_T Internal_DSTATE_l[8];         /* '<S23>/Internal' */
  real_T Memory_PreviousInput;         /* '<S4>/Memory' */
  real_T Memory_PreviousInput_g;       /* '<S13>/Memory' */
  real_T fileID;                       /* '<S6>/SPERTE_measurement_function' */
  real_T Dctpd_RWORK[2];               /* '<S3>/Dctpd' */
  real_T Dct2lowpass_RWORK[4];         /* '<S3>/Dct2lowpass' */
  real_T Dctpd2_RWORK[2];              /* '<S16>/Dctpd2' */
  real_T Dct2lowpass3_RWORK[4];        /* '<S16>/Dct2lowpass3' */
  real_T Dctintegrator1_RWORK[2];      /* '<S16>/Dctintegrator1' */
  real_T Dctpd2_RWORK_p[2];            /* '<S15>/Dctpd2' */
  real_T Dct2lowpass3_RWORK_c[4];      /* '<S15>/Dct2lowpass3' */
  FILE* eml_openfiles[20];             /* '<S6>/SPERTE_measurement_function' */
  int32_T sfEvent;                     /* '<S9>/Supervisor' */
  int32_T sfEvent_f;                   /* '<S6>/SPERTE_measurement_function' */
  uint32_T NS;                         /* '<S6>/SPERTE_measurement_function' */
  int16_T NF;                          /* '<S6>/SPERTE_measurement_function' */
  uint16_T temporalCounter_i1;         /* '<S9>/Supervisor' */
  boolean_T UnitDelay1_DSTATE;         /* '<S9>/Unit Delay1' */
  int8_T DiscreteTimeIntegrator_PrevRese;/* '<S9>/Discrete-Time Integrator' */
  int8_T RepetitiveController_SubsysRanB;/* '<Root>/Repetitive Controller' */
  int8_T Reference_SubsysRanBC;        /* '<Root>/Reference' */
  uint8_T is_active_c1_SPERTE_printer_RC;/* '<S9>/Supervisor' */
  uint8_T busy;                        /* '<S6>/SPERTE_measurement_function' */
  boolean_T doneDoubleBufferReInit;    /* '<S6>/SPERTE_measurement_function' */
  boolean_T fileID_not_empty;          /* '<S6>/SPERTE_measurement_function' */
  boolean_T NF_not_empty;              /* '<S6>/SPERTE_measurement_function' */
  boolean_T busy_not_empty;            /* '<S6>/SPERTE_measurement_function' */
  boolean_T NS_not_empty;              /* '<S6>/SPERTE_measurement_function' */
  boolean_T eml_openfiles_not_empty;   /* '<S6>/SPERTE_measurement_function' */
  boolean_T eml_autoflush[20];         /* '<S6>/SPERTE_measurement_function' */
  boolean_T eml_autoflush_not_empty;   /* '<S6>/SPERTE_measurement_function' */
  boolean_T RepetitiveController_MODE; /* '<Root>/Repetitive Controller' */
  boolean_T Reference_MODE;            /* '<Root>/Reference' */
} DW_SPERTE_printer_RC_T;

/* Zero-crossing (trigger) state */
typedef struct {
  ZCSigState Homingreference_Reset_ZCE;/* '<S9>/Homing reference' */
} PrevZCX_SPERTE_printer_RC_T;

/* Parameters (default storage) */
struct P_SPERTE_printer_RC_T_ {
  real_T N;                            /* Variable: N
                                        * Referenced by:
                                        *   '<S4>/Constant'
                                        *   '<S19>/Constant'
                                        *   '<S21>/Constant'
                                        */
  real_T N_RC_activate;                /* Variable: N_RC_activate
                                        * Referenced by: '<S20>/Constant'
                                        */
  real_T N_trial;                      /* Variable: N_trial
                                        * Referenced by:
                                        *   '<S19>/Constant'
                                        *   '<S21>/Constant'
                                        */
  real_T DiscreteDerivative_ICPrevScaled;
                              /* Mask Parameter: DiscreteDerivative_ICPrevScaled
                               * Referenced by: '<S18>/UD'
                               */
  real_T CompareToConstant_const;     /* Mask Parameter: CompareToConstant_const
                                       * Referenced by: '<S17>/Constant'
                                       */
  uint32_T MeasurementBlock_N_samples;
                                   /* Mask Parameter: MeasurementBlock_N_samples
                                    * Referenced by: '<S6>/SPERTE_measurement_samples'
                                    */
  uint8_T MeasurementBlock_trigger_comman;
                              /* Mask Parameter: MeasurementBlock_trigger_comman
                               * Referenced by: '<S6>/SPERTE_measurement_trigger_command'
                               */
  uint8_T MeasurementBlock_triggertype;
                                 /* Mask Parameter: MeasurementBlock_triggertype
                                  * Referenced by: '<S6>/SPERTE_measurement_function'
                                  */
  real_T homing_ref_Value[600];
  /* Expression: [0           0  1.2531e-05  3.7594e-05  7.5188e-05  0.00012531  0.00018797  0.00026316  0.00035088  0.00045113  0.00056391  0.00068922  0.00082707  0.00097744   0.0011404   0.0013158   0.0015038   0.0017043   0.0019173   0.0021429    0.002381   0.0026316   0.0028947   0.0031704   0.0034586   0.0037594   0.0040727   0.0043985   0.0047368   0.0050877   0.0054511   0.0058271   0.0062155   0.0066165   0.0070301   0.0074561   0.0078947   0.0083459   0.0088095   0.0092857   0.0097744    0.010276    0.010789    0.011316    0.011855    0.012406     0.01297    0.013546    0.014135    0.014737    0.015351    0.015977    0.016617    0.017268    0.017932    0.018609    0.019298        0.02    0.020714    0.021441     0.02218    0.022932    0.023697    0.024474    0.025263    0.026065     0.02688    0.027707    0.028546    0.029398    0.030263     0.03114     0.03203    0.032932    0.033847    0.034774    0.035714    0.036667    0.037632    0.038609    0.039599    0.040602    0.041617    0.042644    0.043684    0.044737    0.045802     0.04688     0.04797    0.049073    0.050188    0.051316    0.052456    0.053609    0.054774    0.055952    0.057143    0.058346    0.059561    0.060789     0.06203    0.063283    0.064549    0.065827    0.067118    0.068421    0.069737    0.071065    0.072406    0.073759    0.075125    0.076504    0.077895    0.079298    0.080714    0.082143    0.083584    0.085038    0.086504    0.087982    0.089474    0.090977    0.092494    0.094023    0.095564    0.097118    0.098684     0.10026     0.10185     0.10346     0.10508      0.1067     0.10835        0.11     0.11167     0.11335     0.11504     0.11674     0.11846     0.12019     0.12193     0.12368     0.12545     0.12723     0.12902     0.13083     0.13264     0.13447     0.13632     0.13817     0.14004     0.14192     0.14381     0.14571     0.14763     0.14956      0.1515     0.15346     0.15543     0.15741      0.1594      0.1614     0.16342     0.16545     0.16749     0.16955     0.17162      0.1737     0.17579     0.17789     0.18001     0.18214     0.18429     0.18644     0.18861     0.19079     0.19298     0.19519     0.19741     0.19964     0.20188     0.20414      0.2064     0.20868     0.21098     0.21328      0.2156     0.21793     0.22028     0.22263       0.225     0.22738     0.22977     0.23218      0.2346     0.23703     0.23947     0.24193      0.2444     0.24688     0.24937     0.25188     0.25439     0.25689      0.2594      0.2619     0.26441     0.26692     0.26942     0.27193     0.27444     0.27694     0.27945     0.28195     0.28446     0.28697     0.28947     0.29198     0.29449     0.29699      0.2995     0.30201     0.30451     0.30702     0.30952     0.31203     0.31454     0.31704     0.31955     0.32206     0.32456     0.32707     0.32957     0.33208     0.33459     0.33709      0.3396     0.34211     0.34461     0.34712     0.34962     0.35213     0.35464     0.35714     0.35965     0.36216     0.36466     0.36717     0.36967     0.37218     0.37469     0.37719      0.3797     0.38221     0.38471     0.38722     0.38972     0.39223     0.39474     0.39724     0.39975     0.40226     0.40476     0.40727     0.40977     0.41228     0.41479     0.41729      0.4198     0.42231     0.42481     0.42732     0.42982     0.43233     0.43484     0.43734     0.43985     0.44236     0.44486     0.44737     0.44987     0.45238     0.45489     0.45739      0.4599     0.46241     0.46491     0.46742     0.46992     0.47243     0.47494     0.47744     0.47995     0.48246     0.48496     0.48747     0.48997     0.49248     0.49499     0.49749         0.5     0.50251     0.50501     0.50752     0.51003     0.51253     0.51504     0.51754     0.52005     0.52256     0.52506     0.52757     0.53008     0.53258     0.53509     0.53759      0.5401     0.54261     0.54511     0.54762     0.55013     0.55263     0.55514     0.55764     0.56015     0.56266     0.56516     0.56767     0.57018     0.57268     0.57519     0.57769      0.5802     0.58271     0.58521     0.58772     0.59023     0.59273     0.59524     0.59774     0.60025     0.60276     0.60526     0.60777     0.61028     0.61278     0.61529     0.61779      0.6203     0.62281     0.62531     0.62782     0.63033     0.63283     0.63534     0.63784     0.64035     0.64286     0.64536     0.64787     0.65038     0.65288     0.65539     0.65789      0.6604     0.66291     0.66541     0.66792     0.67043     0.67293     0.67544     0.67794     0.68045     0.68296     0.68546     0.68797     0.69048     0.69298     0.69549     0.69799      0.7005     0.70301     0.70551     0.70802     0.71053     0.71303     0.71554     0.71805     0.72055     0.72306     0.72556     0.72807     0.73058     0.73308     0.73559      0.7381      0.7406     0.74311     0.74561     0.74812     0.75063     0.75312      0.7556     0.75807     0.76053     0.76297      0.7654     0.76782     0.77023     0.77262       0.775     0.77737     0.77972     0.78207      0.7844     0.78672     0.78902     0.79132      0.7936     0.79586     0.79812     0.80036     0.80259     0.80481     0.80702     0.80921     0.81139     0.81356     0.81571     0.81786     0.81999     0.82211     0.82421      0.8263     0.82838     0.83045     0.83251     0.83455     0.83658      0.8386      0.8406     0.84259     0.84457     0.84654      0.8485     0.85044     0.85237     0.85429     0.85619     0.85808     0.85996     0.86183     0.86368     0.86553     0.86736     0.86917     0.87098     0.87277     0.87455     0.87632     0.87807     0.87981     0.88154     0.88326     0.88496     0.88665     0.88833        0.89     0.89165      0.8933     0.89492     0.89654     0.89815     0.89974     0.90132     0.90288     0.90444     0.90598     0.90751     0.90902     0.91053     0.91202      0.9135     0.91496     0.91642     0.91786     0.91929      0.9207     0.92211      0.9235     0.92487     0.92624     0.92759     0.92893     0.93026     0.93158     0.93288     0.93417     0.93545     0.93672     0.93797     0.93921     0.94044     0.94165     0.94286     0.94405     0.94523     0.94639     0.94754     0.94868     0.94981     0.95093     0.95203     0.95312      0.9542     0.95526     0.95632     0.95736     0.95838      0.9594      0.9604     0.96139     0.96237     0.96333     0.96429     0.96523     0.96615     0.96707     0.96797     0.96886     0.96974      0.9706     0.97145     0.97229     0.97312     0.97393     0.97474     0.97553      0.9763     0.97707     0.97782     0.97856     0.97929        0.98      0.9807     0.98139     0.98207     0.98273     0.98338     0.98402     0.98465     0.98526     0.98586     0.98645     0.98703     0.98759     0.98815     0.98868     0.98921     0.98972     0.99023     0.99071     0.99119     0.99165     0.99211     0.99254     0.99297     0.99338     0.99378     0.99417     0.99455     0.99491     0.99526      0.9956     0.99593     0.99624     0.99654     0.99683     0.99711     0.99737     0.99762     0.99786     0.99808      0.9983      0.9985     0.99868     0.99886     0.99902     0.99917     0.99931     0.99944     0.99955     0.99965     0.99974     0.99981     0.99987     0.99992     0.99996     0.99999           1]'
   * Referenced by: '<S13>/homing_ref'
   */
  real_T Memory_InitialCondition;      /* Expression: 0
                                        * Referenced by: '<S13>/Memory'
                                        */
  real_T One_Value;                    /* Expression: 1
                                        * Referenced by: '<S13>/One'
                                        */
  real_T HomingRunningSwitch_Threshold;/* Expression: 0.5
                                        * Referenced by: '<S9>/Homing Running Switch'
                                        */
  real_T simulationTime_Y0;            /* Expression: 0
                                        * Referenced by: '<S4>/simulationTime'
                                        */
  real_T reference_Y0;                 /* Expression: 0
                                        * Referenced by: '<S4>/reference'
                                        */
  real_T set_Ref_Value[1808];
  /* Expression: [0  9.9981e-10  4.9991e-09  1.4997e-08  3.4993e-08  6.9987e-08  1.2598e-07  2.0996e-07  3.2994e-07  4.9491e-07  7.1487e-07  1.0008e-06  1.3647e-06  1.8197e-06  2.3796e-06  3.0594e-06  3.8753e-06  4.8441e-06  5.9839e-06  7.3136e-06  8.8533e-06  1.0624e-05  1.2648e-05  1.4947e-05  1.7547e-05  2.0471e-05  2.3747e-05    2.74e-05  3.1459e-05  3.5953e-05  4.0912e-05  4.6367e-05   5.235e-05  5.8894e-05  6.6033e-05  7.3801e-05  8.2236e-05  9.1373e-05  0.00010125  0.00011191  0.00012339  0.00013573  0.00014897  0.00016315  0.00017833  0.00019454  0.00021184  0.00023026  0.00024985  0.00027067  0.00029277  0.00031619  0.00034099  0.00036722  0.00039494  0.00042419  0.00045504  0.00048754  0.00052176  0.00055774  0.00059555  0.00063526  0.00067691  0.00072058  0.00076633  0.00081422  0.00086431  0.00091665  0.00097131   0.0010283   0.0010878   0.0011497   0.0012142   0.0012813    0.001351   0.0014234   0.0014986   0.0015765   0.0016573    0.001741   0.0018276   0.0019172   0.0020098   0.0021054   0.0022042   0.0023062   0.0024113   0.0025196   0.0026312   0.0027461   0.0028644    0.002986    0.003111   0.0032395   0.0033715   0.0035069   0.0036459   0.0037884   0.0039346   0.0040843   0.0042377   0.0043948   0.0045555     0.00472   0.0048882   0.0050602   0.0052359   0.0054154   0.0055988    0.005786    0.005977   0.0061719   0.0063707   0.0065734   0.0067799   0.0069904   0.0072048   0.0074232   0.0076454   0.0078717   0.0081018    0.008336   0.0085741   0.0088161   0.0090622   0.0093122   0.0095661   0.0098241    0.010086    0.010352    0.010622    0.010895    0.011173    0.011455     0.01174     0.01203    0.012323    0.012621    0.012922    0.013227    0.013536    0.013849    0.014165    0.014486     0.01481    0.015138     0.01547    0.015806    0.016145    0.016488    0.016834    0.017185    0.017538    0.017896    0.018257    0.018621     0.01899    0.019361    0.019736    0.020114    0.020496    0.020881     0.02127    0.021661    0.022056    0.022454    0.022855     0.02326    0.023667    0.024078    0.024491    0.024907    0.025327    0.025749    0.026173    0.026601    0.027031    0.027464      0.0279    0.028338    0.028778    0.029221    0.029667    0.030114    0.030564    0.031016    0.031471    0.031927    0.032385    0.032846    0.033308    0.033772    0.034238    0.034705    0.035174    0.035645    0.036117    0.036591    0.037066    0.037543    0.038021      0.0385     0.03898    0.039462    0.039945    0.040428    0.040913    0.041399    0.041886    0.042373    0.042862    0.043351    0.043841    0.044332    0.044824    0.045316    0.045809    0.046302    0.046796    0.047291    0.047786    0.048281    0.048777    0.049274     0.04977    0.050268    0.050765    0.051263    0.051761    0.052259    0.052757    0.053256    0.053755    0.054254    0.054753    0.055252    0.055752    0.056251    0.056751    0.057251     0.05775     0.05825     0.05875     0.05925     0.05975     0.06025     0.06075     0.06125     0.06175     0.06225     0.06275     0.06325     0.06375     0.06425     0.06475     0.06525     0.06575     0.06625     0.06675     0.06725     0.06775     0.06825     0.06875     0.06925     0.06975     0.07025     0.07075     0.07125     0.07175     0.07225     0.07275     0.07325     0.07375     0.07425     0.07475     0.07525     0.07575     0.07625     0.07675     0.07725     0.07775     0.07825     0.07875     0.07925     0.07975     0.08025     0.08075     0.08125     0.08175     0.08225     0.08275     0.08325     0.08375     0.08425     0.08475     0.08525     0.08575     0.08625     0.08675     0.08725     0.08775     0.08825     0.08875     0.08925     0.08975     0.09025     0.09075     0.09125     0.09175     0.09225     0.09275     0.09325     0.09375     0.09425     0.09475     0.09525     0.09575     0.09625     0.09675     0.09725     0.09775     0.09825     0.09875     0.09925     0.09975     0.10025     0.10075     0.10125     0.10175     0.10225     0.10275     0.10325     0.10375     0.10425     0.10475     0.10525     0.10575     0.10625     0.10675     0.10725     0.10775     0.10825     0.10875     0.10925     0.10975     0.11025     0.11075     0.11125     0.11175     0.11225     0.11275     0.11325     0.11375     0.11425     0.11475     0.11525     0.11575     0.11625     0.11675     0.11725     0.11775     0.11825     0.11875     0.11925     0.11975     0.12025     0.12075     0.12125     0.12175     0.12225     0.12275     0.12325     0.12375     0.12425     0.12475     0.12525     0.12575     0.12625     0.12675     0.12725     0.12775     0.12825     0.12875     0.12925     0.12975     0.13025     0.13075     0.13125     0.13175     0.13225     0.13275     0.13325     0.13375     0.13425     0.13475     0.13525     0.13575     0.13625     0.13675     0.13725     0.13775     0.13825     0.13875     0.13925     0.13975     0.14025     0.14075     0.14125     0.14175     0.14225     0.14275     0.14325     0.14375     0.14425     0.14475     0.14525     0.14575     0.14625     0.14675     0.14725     0.14775     0.14825     0.14875     0.14925     0.14975     0.15025     0.15075     0.15125     0.15175     0.15225     0.15275     0.15325     0.15375     0.15425     0.15475     0.15525     0.15575     0.15625     0.15675     0.15725     0.15775     0.15825     0.15875     0.15925     0.15975     0.16025     0.16075     0.16125     0.16175     0.16225     0.16275     0.16325     0.16375     0.16425     0.16475     0.16525     0.16575     0.16625     0.16675     0.16725     0.16775     0.16825     0.16875     0.16925     0.16975     0.17025     0.17075     0.17125     0.17175     0.17225     0.17275     0.17325     0.17375     0.17425     0.17475     0.17525     0.17575     0.17625     0.17675     0.17725     0.17775     0.17825     0.17875     0.17925     0.17975     0.18025     0.18075     0.18125     0.18175     0.18225     0.18275     0.18325     0.18375     0.18425     0.18475     0.18525     0.18575     0.18625     0.18675     0.18725     0.18775     0.18825     0.18875     0.18925     0.18975     0.19025     0.19075     0.19125     0.19175     0.19225     0.19275     0.19325     0.19375     0.19425     0.19475     0.19525     0.19575     0.19625     0.19675     0.19725     0.19775     0.19825     0.19875     0.19925     0.19975     0.20025     0.20075     0.20125     0.20175     0.20225     0.20275     0.20325     0.20375     0.20425     0.20475     0.20525     0.20575     0.20625     0.20675     0.20725     0.20775     0.20825     0.20875     0.20925     0.20975     0.21025     0.21075     0.21125     0.21175     0.21225     0.21275     0.21325     0.21375     0.21425     0.21475     0.21525     0.21575     0.21625     0.21675     0.21725     0.21775     0.21825     0.21875     0.21925     0.21975     0.22025     0.22075     0.22125     0.22175     0.22225     0.22275     0.22325     0.22375     0.22425     0.22475     0.22525     0.22575     0.22625     0.22675     0.22725     0.22775     0.22825     0.22875     0.22925     0.22975     0.23025     0.23075     0.23125     0.23175     0.23225     0.23275     0.23325     0.23375     0.23425     0.23475     0.23525     0.23575     0.23625     0.23675     0.23725     0.23775     0.23825     0.23875     0.23925     0.23975     0.24025     0.24075     0.24125     0.24175     0.24225     0.24275     0.24325     0.24375     0.24425     0.24475     0.24525     0.24575     0.24625     0.24674     0.24724     0.24774     0.24824     0.24874     0.24924     0.24973     0.25023     0.25073     0.25122     0.25172     0.25221     0.25271      0.2532      0.2537     0.25419     0.25468     0.25518     0.25567     0.25616     0.25665     0.25714     0.25763     0.25811      0.2586     0.25909     0.25957     0.26006     0.26054     0.26102      0.2615     0.26198     0.26246     0.26293     0.26341     0.26388     0.26436     0.26483     0.26529     0.26576     0.26623     0.26669     0.26715     0.26761     0.26807     0.26853     0.26898     0.26944     0.26989     0.27033     0.27078     0.27122     0.27166      0.2721     0.27254     0.27297      0.2734     0.27383     0.27425     0.27467     0.27509     0.27551     0.27592     0.27633     0.27674     0.27714     0.27755     0.27794     0.27834     0.27873     0.27912      0.2795     0.27989     0.28026     0.28064     0.28101     0.28138     0.28174      0.2821     0.28246     0.28282     0.28317     0.28351     0.28386     0.28419     0.28453     0.28486     0.28519     0.28551     0.28583     0.28615     0.28646     0.28677     0.28708     0.28738     0.28768     0.28797     0.28826     0.28855     0.28883      0.2891     0.28938     0.28965     0.28991     0.29018     0.29043     0.29069     0.29094     0.29118     0.29143     0.29166      0.2919     0.29213     0.29235     0.29258      0.2928     0.29301     0.29322     0.29343     0.29363     0.29383     0.29402     0.29421      0.2944     0.29458     0.29476     0.29494     0.29511     0.29528     0.29544     0.29561     0.29576     0.29592     0.29607     0.29621     0.29635     0.29649     0.29663     0.29676     0.29689     0.29701     0.29714     0.29725     0.29737     0.29748     0.29759     0.29769      0.2978     0.29789     0.29799     0.29808     0.29817     0.29826     0.29834     0.29842      0.2985     0.29858     0.29865     0.29872     0.29879     0.29885     0.29891     0.29897     0.29903     0.29908     0.29914     0.29919     0.29923     0.29928     0.29932     0.29936      0.2994     0.29944     0.29948     0.29951     0.29954     0.29958     0.29961     0.29963     0.29966     0.29968     0.29971     0.29973     0.29975     0.29977     0.29979     0.29981     0.29982     0.29984     0.29985     0.29986     0.29988     0.29989      0.2999     0.29991     0.29992     0.29993     0.29993     0.29994     0.29995     0.29995     0.29996     0.29996     0.29997     0.29997     0.29998     0.29998     0.29998     0.29999     0.29999     0.29999     0.29999     0.29999     0.29999         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3         0.3     0.29999     0.29999     0.29999     0.29999     0.29999     0.29999     0.29998     0.29998     0.29998     0.29997     0.29997     0.29996     0.29996     0.29995     0.29995     0.29994     0.29993     0.29993     0.29992     0.29991      0.2999     0.29989     0.29988     0.29986     0.29985     0.29984     0.29982     0.29981     0.29979     0.29977     0.29975     0.29973     0.29971     0.29968     0.29966     0.29963     0.29961     0.29958     0.29954     0.29951     0.29948     0.29944      0.2994     0.29936     0.29932     0.29928     0.29923     0.29919     0.29914     0.29908     0.29903     0.29897     0.29891     0.29885     0.29879     0.29872     0.29865     0.29858      0.2985     0.29842     0.29834     0.29826     0.29817     0.29808     0.29799     0.29789      0.2978     0.29769     0.29759     0.29748     0.29737     0.29725     0.29714     0.29701     0.29689     0.29676     0.29663     0.29649     0.29635     0.29621     0.29607     0.29592     0.29576     0.29561     0.29544     0.29528     0.29511     0.29494     0.29476     0.29458      0.2944     0.29421     0.29402     0.29383     0.29363     0.29343     0.29322     0.29301      0.2928     0.29258     0.29235     0.29213      0.2919     0.29166     0.29143     0.29118     0.29094     0.29069     0.29043     0.29018     0.28991     0.28965     0.28938      0.2891     0.28883     0.28855     0.28826     0.28797     0.28768     0.28738     0.28708     0.28677     0.28646     0.28615     0.28583     0.28551     0.28519     0.28486     0.28453     0.28419     0.28386     0.28351     0.28317     0.28282     0.28246      0.2821     0.28174     0.28138     0.28101     0.28064     0.28026     0.27989      0.2795     0.27912     0.27873     0.27834     0.27794     0.27755     0.27714     0.27674     0.27633     0.27592     0.27551     0.27509     0.27467     0.27425     0.27383      0.2734     0.27297     0.27254      0.2721     0.27166     0.27122     0.27078     0.27033     0.26989     0.26944     0.26898     0.26853     0.26807     0.26761     0.26715     0.26669     0.26623     0.26576     0.26529     0.26483     0.26436     0.26388     0.26341     0.26293     0.26246     0.26198      0.2615     0.26102     0.26054     0.26006     0.25957     0.25909      0.2586     0.25811     0.25763     0.25714     0.25665     0.25616     0.25567     0.25518     0.25468     0.25419      0.2537      0.2532     0.25271     0.25221     0.25172     0.25122     0.25073     0.25023     0.24973     0.24924     0.24874     0.24824     0.24774     0.24724     0.24674     0.24625     0.24575     0.24525     0.24475     0.24425     0.24375     0.24325     0.24275     0.24225     0.24175     0.24125     0.24075     0.24025     0.23975     0.23925     0.23875     0.23825     0.23775     0.23725     0.23675     0.23625     0.23575     0.23525     0.23475     0.23425     0.23375     0.23325     0.23275     0.23225     0.23175     0.23125     0.23075     0.23025     0.22975     0.22925     0.22875     0.22825     0.22775     0.22725     0.22675     0.22625     0.22575     0.22525     0.22475     0.22425     0.22375     0.22325     0.22275     0.22225     0.22175     0.22125     0.22075     0.22025     0.21975     0.21925     0.21875     0.21825     0.21775     0.21725     0.21675     0.21625     0.21575     0.21525     0.21475     0.21425     0.21375     0.21325     0.21275     0.21225     0.21175     0.21125     0.21075     0.21025     0.20975     0.20925     0.20875     0.20825     0.20775     0.20725     0.20675     0.20625     0.20575     0.20525     0.20475     0.20425     0.20375     0.20325     0.20275     0.20225     0.20175     0.20125     0.20075     0.20025     0.19975     0.19925     0.19875     0.19825     0.19775     0.19725     0.19675     0.19625     0.19575     0.19525     0.19475     0.19425     0.19375     0.19325     0.19275     0.19225     0.19175     0.19125     0.19075     0.19025     0.18975     0.18925     0.18875     0.18825     0.18775     0.18725     0.18675     0.18625     0.18575     0.18525     0.18475     0.18425     0.18375     0.18325     0.18275     0.18225     0.18175     0.18125     0.18075     0.18025     0.17975     0.17925     0.17875     0.17825     0.17775     0.17725     0.17675     0.17625     0.17575     0.17525     0.17475     0.17425     0.17375     0.17325     0.17275     0.17225     0.17175     0.17125     0.17075     0.17025     0.16975     0.16925     0.16875     0.16825     0.16775     0.16725     0.16675     0.16625     0.16575     0.16525     0.16475     0.16425     0.16375     0.16325     0.16275     0.16225     0.16175     0.16125     0.16075     0.16025     0.15975     0.15925     0.15875     0.15825     0.15775     0.15725     0.15675     0.15625     0.15575     0.15525     0.15475     0.15425     0.15375     0.15325     0.15275     0.15225     0.15175     0.15125     0.15075     0.15025     0.14975     0.14925     0.14875     0.14825     0.14775     0.14725     0.14675     0.14625     0.14575     0.14525     0.14475     0.14425     0.14375     0.14325     0.14275     0.14225     0.14175     0.14125     0.14075     0.14025     0.13975     0.13925     0.13875     0.13825     0.13775     0.13725     0.13675     0.13625     0.13575     0.13525     0.13475     0.13425     0.13375     0.13325     0.13275     0.13225     0.13175     0.13125     0.13075     0.13025     0.12975     0.12925     0.12875     0.12825     0.12775     0.12725     0.12675     0.12625     0.12575     0.12525     0.12475     0.12425     0.12375     0.12325     0.12275     0.12225     0.12175     0.12125     0.12075     0.12025     0.11975     0.11925     0.11875     0.11825     0.11775     0.11725     0.11675     0.11625     0.11575     0.11525     0.11475     0.11425     0.11375     0.11325     0.11275     0.11225     0.11175     0.11125     0.11075     0.11025     0.10975     0.10925     0.10875     0.10825     0.10775     0.10725     0.10675     0.10625     0.10575     0.10525     0.10475     0.10425     0.10375     0.10325     0.10275     0.10225     0.10175     0.10125     0.10075     0.10025     0.09975     0.09925     0.09875     0.09825     0.09775     0.09725     0.09675     0.09625     0.09575     0.09525     0.09475     0.09425     0.09375     0.09325     0.09275     0.09225     0.09175     0.09125     0.09075     0.09025     0.08975     0.08925     0.08875     0.08825     0.08775     0.08725     0.08675     0.08625     0.08575     0.08525     0.08475     0.08425     0.08375     0.08325     0.08275     0.08225     0.08175     0.08125     0.08075     0.08025     0.07975     0.07925     0.07875     0.07825     0.07775     0.07725     0.07675     0.07625     0.07575     0.07525     0.07475     0.07425     0.07375     0.07325     0.07275     0.07225     0.07175     0.07125     0.07075     0.07025     0.06975     0.06925     0.06875     0.06825     0.06775     0.06725     0.06675     0.06625     0.06575     0.06525     0.06475     0.06425     0.06375     0.06325     0.06275     0.06225     0.06175     0.06125     0.06075     0.06025     0.05975     0.05925     0.05875     0.05825     0.05775    0.057251    0.056751    0.056251    0.055752    0.055252    0.054753    0.054254    0.053755    0.053256    0.052757    0.052259    0.051761    0.051263    0.050765    0.050268     0.04977    0.049274    0.048777    0.048281    0.047786    0.047291    0.046796    0.046302    0.045809    0.045316    0.044824    0.044332    0.043841    0.043351    0.042862    0.042373    0.041886    0.041399    0.040913    0.040428    0.039945    0.039462     0.03898      0.0385    0.038021    0.037543    0.037066    0.036591    0.036117    0.035645    0.035174    0.034705    0.034238    0.033772    0.033308    0.032846    0.032385    0.031927    0.031471    0.031016    0.030564    0.030114    0.029667    0.029221    0.028778    0.028338      0.0279    0.027464    0.027031    0.026601    0.026173    0.025749    0.025327    0.024907    0.024491    0.024078    0.023667     0.02326    0.022855    0.022454    0.022056    0.021661     0.02127    0.020881    0.020496    0.020114    0.019736    0.019361     0.01899    0.018621    0.018257    0.017896    0.017538    0.017185    0.016834    0.016488    0.016145    0.015806     0.01547    0.015138     0.01481    0.014486    0.014165    0.013849    0.013536    0.013227    0.012922    0.012621    0.012323     0.01203     0.01174    0.011455    0.011173    0.010895    0.010622    0.010352    0.010086   0.0098241   0.0095661   0.0093122   0.0090622   0.0088161   0.0085741    0.008336   0.0081018   0.0078717   0.0076454   0.0074232   0.0072048   0.0069904   0.0067799   0.0065734   0.0063707   0.0061719    0.005977    0.005786   0.0055988   0.0054154   0.0052359   0.0050602   0.0048882     0.00472   0.0045555   0.0043948   0.0042377   0.0040843   0.0039346   0.0037884   0.0036459   0.0035069   0.0033715   0.0032395    0.003111    0.002986   0.0028644   0.0027461   0.0026312   0.0025196   0.0024113   0.0023062   0.0022042   0.0021054   0.0020098   0.0019172   0.0018276    0.001741   0.0016573   0.0015765   0.0014986   0.0014234    0.001351   0.0012813   0.0012142   0.0011497   0.0010878   0.0010283  0.00097131  0.00091665  0.00086431  0.00081422  0.00076633  0.00072058  0.00067691  0.00063526  0.00059555  0.00055774  0.00052176  0.00048754  0.00045504  0.00042419  0.00039494  0.00036722  0.00034099  0.00031619  0.00029277  0.00027067  0.00024985  0.00023026  0.00021184  0.00019454  0.00017833  0.00016315  0.00014897  0.00013573  0.00012339  0.00011191  0.00010125  9.1373e-05  8.2236e-05  7.3801e-05  6.6033e-05  5.8894e-05   5.235e-05  4.6367e-05  4.0912e-05  3.5953e-05  3.1459e-05    2.74e-05  2.3747e-05  2.0471e-05  1.7547e-05  1.4947e-05  1.2648e-05  1.0624e-05  8.8533e-06  7.3136e-06  5.9839e-06  4.8441e-06  3.8753e-06  3.0594e-06  2.3796e-06  1.8197e-06  1.3647e-06  1.0008e-06  7.1487e-07  4.9491e-07  3.2994e-07  2.0996e-07  1.2598e-07  6.9987e-08  3.4993e-08  1.4997e-08  4.9991e-09  9.9981e-10           0           0           0           0           0           0           0           0           0]'
   * Referenced by: '<S4>/set_Ref'
   */
  real_T Memory_InitialCondition_c;    /* Expression: 0
                                        * Referenced by: '<S4>/Memory'
                                        */
  real_T One_Value_h;                  /* Expression: 1
                                        * Referenced by: '<S4>/One'
                                        */
  real_T Gain_Gain;                    /* Expression: 1/1000
                                        * Referenced by: '<S4>/Gain'
                                        */
  real_T Constant1_Value;              /* Expression: 0.001
                                        * Referenced by: '<S4>/Constant1'
                                        */
  real_T Output_Y0;                    /* Expression: [0]
                                        * Referenced by: '<S5>/Output'
                                        */
  real_T Delay_InitialCondition;       /* Expression: 0
                                        * Referenced by: '<S5>/Delay'
                                        */
  real_T Delay1_InitialCondition;      /* Expression: 0
                                        * Referenced by: '<S5>/Delay1'
                                        */
  real_T Internal_A[100];              /* Computed Parameter: Internal_A
                                        * Referenced by: '<S22>/Internal'
                                        */
  real_T Internal_C[100];              /* Computed Parameter: Internal_C
                                        * Referenced by: '<S22>/Internal'
                                        */
  real_T Internal_D;                   /* Computed Parameter: Internal_D
                                        * Referenced by: '<S22>/Internal'
                                        */
  real_T Internal_InitialCondition;    /* Expression: xinit
                                        * Referenced by: '<S22>/Internal'
                                        */
  real_T Internal_A_f[20];             /* Computed Parameter: Internal_A_f
                                        * Referenced by: '<S23>/Internal'
                                        */
  real_T Internal_B[4];                /* Computed Parameter: Internal_B
                                        * Referenced by: '<S23>/Internal'
                                        */
  real_T Internal_C_m[8];              /* Computed Parameter: Internal_C_m
                                        * Referenced by: '<S23>/Internal'
                                        */
  real_T Internal_D_m;                 /* Computed Parameter: Internal_D_m
                                        * Referenced by: '<S23>/Internal'
                                        */
  real_T Internal_InitialCondition_g;  /* Expression: xinit
                                        * Referenced by: '<S23>/Internal'
                                        */
  real_T SFunction_P1_Size[2];         /* Computed Parameter: SFunction_P1_Size
                                        * Referenced by: '<S11>/S-Function'
                                        */
  real_T SFunction_P1;                 /* Expression: portid
                                        * Referenced by: '<S11>/S-Function'
                                        */
  real_T SFunction_P2_Size[2];         /* Computed Parameter: SFunction_P2_Size
                                        * Referenced by: '<S11>/S-Function'
                                        */
  real_T SFunction_P2;                 /* Expression: ectimeout
                                        * Referenced by: '<S11>/S-Function'
                                        */
  real_T ec_Ebox_P1_Size[2];           /* Computed Parameter: ec_Ebox_P1_Size
                                        * Referenced by: '<S10>/ec_Ebox'
                                        */
  real_T ec_Ebox_P1;                   /* Expression: link_id
                                        * Referenced by: '<S10>/ec_Ebox'
                                        */
  real_T Gain_Gain_k;                  /* Expression: 1/100
                                        * Referenced by: '<S10>/Gain'
                                        */
  real_T Constant2_Value;              /* Expression: 0
                                        * Referenced by: '<S8>/Constant2'
                                        */
  real_T CountstoMeter1_Gain;          /* Expression: 1/(600*100/2.54)
                                        * Referenced by: '<S8>/Counts to Meter1'
                                        */
  real_T Gain_Gain_j;                  /* Expression: 4
                                        * Referenced by: '<S3>/Gain'
                                        */
  real_T Dctpd_P1_Size[2];             /* Computed Parameter: Dctpd_P1_Size
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dctpd_P1;                     /* Expression: kp
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dctpd_P2_Size[2];             /* Computed Parameter: Dctpd_P2_Size
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dctpd_P2;                     /* Expression: kv
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dctpd_P3_Size[2];             /* Computed Parameter: Dctpd_P3_Size
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dctpd_P3;                     /* Expression: 0.001
                                        * Referenced by: '<S3>/Dctpd'
                                        */
  real_T Dct2lowpass_P1_Size[2];      /* Computed Parameter: Dct2lowpass_P1_Size
                                       * Referenced by: '<S3>/Dct2lowpass'
                                       */
  real_T Dct2lowpass_P1;               /* Expression: f_den
                                        * Referenced by: '<S3>/Dct2lowpass'
                                        */
  real_T Dct2lowpass_P2_Size[2];      /* Computed Parameter: Dct2lowpass_P2_Size
                                       * Referenced by: '<S3>/Dct2lowpass'
                                       */
  real_T Dct2lowpass_P2;               /* Expression: b_den
                                        * Referenced by: '<S3>/Dct2lowpass'
                                        */
  real_T Dct2lowpass_P3_Size[2];      /* Computed Parameter: Dct2lowpass_P3_Size
                                       * Referenced by: '<S3>/Dct2lowpass'
                                       */
  real_T Dct2lowpass_P3;               /* Expression: 0.001
                                        * Referenced by: '<S3>/Dct2lowpass'
                                        */
  real_T DiscreteTimeIntegrator_gainval;
                           /* Computed Parameter: DiscreteTimeIntegrator_gainval
                            * Referenced by: '<S9>/Discrete-Time Integrator'
                            */
  real_T DiscreteTimeIntegrator_IC;    /* Expression: 0
                                        * Referenced by: '<S9>/Discrete-Time Integrator'
                                        */
  real_T TSamp_WtEt;                   /* Computed Parameter: TSamp_WtEt
                                        * Referenced by: '<S18>/TSamp'
                                        */
  real_T Gain1_Gain;                   /* Expression: 4
                                        * Referenced by: '<S16>/Gain1'
                                        */
  real_T Dctpd2_P1_Size[2];            /* Computed Parameter: Dctpd2_P1_Size
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dctpd2_P1;                    /* Expression: kp
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dctpd2_P2_Size[2];            /* Computed Parameter: Dctpd2_P2_Size
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dctpd2_P2;                    /* Expression: kv
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dctpd2_P3_Size[2];            /* Computed Parameter: Dctpd2_P3_Size
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dctpd2_P3;                    /* Expression: 0.001
                                        * Referenced by: '<S16>/Dctpd2'
                                        */
  real_T Dct2lowpass3_P1_Size[2];    /* Computed Parameter: Dct2lowpass3_P1_Size
                                      * Referenced by: '<S16>/Dct2lowpass3'
                                      */
  real_T Dct2lowpass3_P1;              /* Expression: f_den
                                        * Referenced by: '<S16>/Dct2lowpass3'
                                        */
  real_T Dct2lowpass3_P2_Size[2];    /* Computed Parameter: Dct2lowpass3_P2_Size
                                      * Referenced by: '<S16>/Dct2lowpass3'
                                      */
  real_T Dct2lowpass3_P2;              /* Expression: b_den
                                        * Referenced by: '<S16>/Dct2lowpass3'
                                        */
  real_T Dct2lowpass3_P3_Size[2];    /* Computed Parameter: Dct2lowpass3_P3_Size
                                      * Referenced by: '<S16>/Dct2lowpass3'
                                      */
  real_T Dct2lowpass3_P3;              /* Expression: 0.001
                                        * Referenced by: '<S16>/Dct2lowpass3'
                                        */
  real_T Dctintegrator1_P1_Size[2];/* Computed Parameter: Dctintegrator1_P1_Size
                                    * Referenced by: '<S16>/Dctintegrator1'
                                    */
  real_T Dctintegrator1_P1;            /* Expression: f_num
                                        * Referenced by: '<S16>/Dctintegrator1'
                                        */
  real_T Dctintegrator1_P2_Size[2];/* Computed Parameter: Dctintegrator1_P2_Size
                                    * Referenced by: '<S16>/Dctintegrator1'
                                    */
  real_T Dctintegrator1_P2;            /* Expression: 0.001
                                        * Referenced by: '<S16>/Dctintegrator1'
                                        */
  real_T Gain1_Gain_j;                 /* Expression: 4
                                        * Referenced by: '<S15>/Gain1'
                                        */
  real_T Dctpd2_P1_Size_b[2];          /* Computed Parameter: Dctpd2_P1_Size_b
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dctpd2_P1_m;                  /* Expression: kp
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dctpd2_P2_Size_l[2];          /* Computed Parameter: Dctpd2_P2_Size_l
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dctpd2_P2_n;                  /* Expression: kv
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dctpd2_P3_Size_g[2];          /* Computed Parameter: Dctpd2_P3_Size_g
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dctpd2_P3_d;                  /* Expression: 0.001
                                        * Referenced by: '<S15>/Dctpd2'
                                        */
  real_T Dct2lowpass3_P1_Size_f[2];/* Computed Parameter: Dct2lowpass3_P1_Size_f
                                    * Referenced by: '<S15>/Dct2lowpass3'
                                    */
  real_T Dct2lowpass3_P1_g;            /* Expression: f_den
                                        * Referenced by: '<S15>/Dct2lowpass3'
                                        */
  real_T Dct2lowpass3_P2_Size_k[2];/* Computed Parameter: Dct2lowpass3_P2_Size_k
                                    * Referenced by: '<S15>/Dct2lowpass3'
                                    */
  real_T Dct2lowpass3_P2_a;            /* Expression: b_den
                                        * Referenced by: '<S15>/Dct2lowpass3'
                                        */
  real_T Dct2lowpass3_P3_Size_j[2];/* Computed Parameter: Dct2lowpass3_P3_Size_j
                                    * Referenced by: '<S15>/Dct2lowpass3'
                                    */
  real_T Dct2lowpass3_P3_i;            /* Expression: 0.001
                                        * Referenced by: '<S15>/Dct2lowpass3'
                                        */
  real_T Saturation_UpperSat;          /* Expression: 2.5
                                        * Referenced by: '<S8>/Saturation'
                                        */
  real_T Saturation_LowerSat;          /* Expression: -2.5
                                        * Referenced by: '<S8>/Saturation'
                                        */
  real_T Saturation_UpperSat_e;        /* Expression: 10
                                        * Referenced by: '<S10>/Saturation'
                                        */
  real_T Saturation_LowerSat_i;        /* Expression: -10
                                        * Referenced by: '<S10>/Saturation'
                                        */
  real_T ResetEncoder_P1_Size[2];    /* Computed Parameter: ResetEncoder_P1_Size
                                      * Referenced by: '<S9>/Reset Encoder'
                                      */
  real_T ResetEncoder_P1;              /* Expression: link_id
                                        * Referenced by: '<S9>/Reset Encoder'
                                        */
  uint16_T RCactivate_Y0;              /* Computed Parameter: RCactivate_Y0
                                        * Referenced by: '<S4>/RCactivate'
                                        */
  boolean_T UnitDelay1_InitialCondition;
                              /* Computed Parameter: UnitDelay1_InitialCondition
                               * Referenced by: '<S9>/Unit Delay1'
                               */
  uint8_T signvBF_Gain;                /* Computed Parameter: signvBF_Gain
                                        * Referenced by: '<S12>/sign(v) BF'
                                        */
};

/* Real-time Model Data Structure */
struct tag_RTM_SPERTE_printer_RC_T {
  struct SimStruct_tag * *childSfunctions;
  const char_T *errorStatus;
  SS_SimMode simMode;
  RTWExtModeInfo *extModeInfo;
  RTWSolverInfo solverInfo;
  RTWSolverInfo *solverInfoPtr;
  void *sfcnInfo;

  /*
   * NonInlinedSFcns:
   * The following substructure contains information regarding
   * non-inlined s-functions used in the model.
   */
  struct {
    RTWSfcnInfo sfcnInfo;
    time_T *taskTimePtrs[2];
    SimStruct childSFunctions[10];
    SimStruct *childSFunctionPtrs[10];
    struct _ssBlkInfo2 blkInfo2[10];
    struct _ssSFcnModelMethods2 methods2[10];
    struct _ssSFcnModelMethods3 methods3[10];
    struct _ssSFcnModelMethods4 methods4[10];
    struct _ssStatesInfo2 statesInfo2[10];
    ssPeriodicStatesInfo periodicStatesInfo[10];
    struct _ssPortInfo2 inputOutputPortInfo2[10];
    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[2];
      mxArray *params[2];
    } Sfcn0;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[3];
      struct _ssInPortUnit inputPortUnits[3];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[3];
      real_T const *UPtrs0[2];
      real_T const *UPtrs1[2];
      real_T const *UPtrs2[8];
      struct _ssPortOutputs outputPortInfo[3];
      struct _ssOutPortUnit outputPortUnits[3];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[3];
      uint_T attribs[1];
      mxArray *params[1];
    } Sfcn1;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn2;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn3;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn4;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn5;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[2];
      struct _ssInPortUnit inputPortUnits[2];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[2];
      real_T const *UPtrs0[1];
      real_T const *UPtrs1[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[2];
      mxArray *params[2];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn6;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn7;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      struct _ssPortOutputs outputPortInfo[1];
      struct _ssOutPortUnit outputPortUnits[1];
      struct _ssOutPortCoSimAttribute outputPortCoSimAttribute[1];
      uint_T attribs[3];
      mxArray *params[3];
      struct _ssDWorkRecord dWork[1];
      struct _ssDWorkAuxRecord dWorkAux[1];
    } Sfcn8;

    struct {
      time_T sfcnPeriod[1];
      time_T sfcnOffset[1];
      int_T sfcnTsMap[1];
      struct _ssPortInputs inputPortInfo[1];
      struct _ssInPortUnit inputPortUnits[1];
      struct _ssInPortCoSimAttribute inputPortCoSimAttribute[1];
      real_T const *UPtrs0[1];
      uint_T attribs[1];
      mxArray *params[1];
    } Sfcn9;
  } NonInlinedSFcns;

  boolean_T zCCacheNeedsReset;
  boolean_T derivCacheNeedsReset;
  boolean_T CTOutputIncnstWithState;

  /*
   * Sizes:
   * The following substructure contains sizes information
   * for many of the model attributes such as inputs, outputs,
   * dwork, sample times, etc.
   */
  struct {
    uint32_T checksums[4];
    uint32_T options;
    int_T numContStates;
    int_T numU;
    int_T numY;
    int_T numSampTimes;
    int_T numBlocks;
    int_T numBlockIO;
    int_T numBlockPrms;
    int_T numDwork;
    int_T numSFcnPrms;
    int_T numSFcns;
    int_T numIports;
    int_T numOports;
    int_T numNonSampZCs;
    int_T sysDirFeedThru;
    int_T rtwGenSfcn;
  } Sizes;

  /*
   * SpecialInfo:
   * The following substructure contains special information
   * related to other components that are dependent on RTW.
   */
  struct {
    const void *mappingInfo;
  } SpecialInfo;

  /*
   * Timing:
   * The following substructure contains information regarding
   * the timing information for the model.
   */
  struct {
    time_T stepSize;
    uint32_T clockTick0;
    uint32_T clockTickH0;
    time_T stepSize0;
    uint32_T clockTick1;
    uint32_T clockTickH1;
    time_T stepSize1;
    time_T tStart;
    time_T tFinal;
    time_T timeOfLastOutput;
    SimTimeStep simTimeStep;
    boolean_T stopRequestedFlag;
    time_T *sampleTimes;
    time_T *offsetTimes;
    int_T *sampleTimeTaskIDPtr;
    int_T *sampleHits;
    int_T *perTaskSampleHits;
    time_T *t;
    time_T sampleTimesArray[2];
    time_T offsetTimesArray[2];
    int_T sampleTimeTaskIDArray[2];
    int_T sampleHitArray[2];
    int_T perTaskSampleHitsArray[4];
    time_T tArray[2];
  } Timing;
};

/* Block parameters (default storage) */
extern P_SPERTE_printer_RC_T SPERTE_printer_RC_P;

/* Block signals (default storage) */
extern B_SPERTE_printer_RC_T SPERTE_printer_RC_B;

/* Block states (default storage) */
extern DW_SPERTE_printer_RC_T SPERTE_printer_RC_DW;

/* Zero-crossing (trigger) state */
extern PrevZCX_SPERTE_printer_RC_T SPERTE_printer_RC_PrevZCX;

/* Model entry point functions */
extern void SPERTE_printer_RC_initialize(void);
extern void SPERTE_printer_RC_step(void);
extern void SPERTE_printer_RC_terminate(void);

/* Real-time Model object */
extern RT_MODEL_SPERTE_printer_RC_T *const SPERTE_printer_RC_M;
extern volatile boolean_T stopRequested;
extern volatile boolean_T runModel;

/*-
 * These blocks were eliminated from the model due to optimizations:
 *
 * Block '<S8>/Rate Transition12' : Unused code path elimination
 * Block '<S8>/Rate Transition13' : Unused code path elimination
 * Block '<S8>/Rate Transition15' : Unused code path elimination
 * Block '<S8>/Rate Transition16' : Unused code path elimination
 * Block '<S8>/Rate Transition17' : Unused code path elimination
 * Block '<S8>/Rate Transition18' : Unused code path elimination
 * Block '<S8>/Rate Transition19' : Unused code path elimination
 * Block '<S8>/Rate Transition20' : Unused code path elimination
 * Block '<S8>/Rate Transition21' : Unused code path elimination
 * Block '<S8>/Rate Transition22' : Unused code path elimination
 * Block '<S18>/Data Type Duplicate' : Unused code path elimination
 * Block '<S8>/Rate Transition14' : Eliminated since input and output rates are identical
 * Block '<S13>/Data Type Conversion' : Eliminate redundant data type conversion
 * Block '<S4>/Data Type Conversion1' : Eliminate redundant data type conversion
 */

/*-
 * The generated code includes comments that allow you to trace directly
 * back to the appropriate location in the model.  The basic format
 * is <system>/block_name, where system is the system number (uniquely
 * assigned by Simulink) and block_name is the name of the block.
 *
 * Use the MATLAB hilite_system command to trace the generated code back
 * to the model.  For example,
 *
 * hilite_system('<S3>')    - opens system 3
 * hilite_system('<S3>/Kp') - opens and selects block Kp which resides in S3
 *
 * Here is the system hierarchy for this model
 *
 * '<Root>' : 'SPERTE_printer_RC'
 * '<S1>'   : 'SPERTE_printer_RC/Measurement'
 * '<S2>'   : 'SPERTE_printer_RC/Printer'
 * '<S3>'   : 'SPERTE_printer_RC/Printer Controller'
 * '<S4>'   : 'SPERTE_printer_RC/Reference'
 * '<S5>'   : 'SPERTE_printer_RC/Repetitive Controller'
 * '<S6>'   : 'SPERTE_printer_RC/Measurement/MeasurementBlock'
 * '<S7>'   : 'SPERTE_printer_RC/Measurement/MeasurementBlock/SPERTE_measurement_function'
 * '<S8>'   : 'SPERTE_printer_RC/Printer/E//BOX I-O'
 * '<S9>'   : 'SPERTE_printer_RC/Printer/Supervisory Controller'
 * '<S10>'  : 'SPERTE_printer_RC/Printer/E//BOX I-O/Ethercat E-box'
 * '<S11>'  : 'SPERTE_printer_RC/Printer/E//BOX I-O/Ethercat Supervisor'
 * '<S12>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/Homing feedforward'
 * '<S13>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/Homing reference'
 * '<S14>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/Supervisor'
 * '<S15>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/airbagController'
 * '<S16>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/homingController'
 * '<S17>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/Homing feedforward/Compare To Constant'
 * '<S18>'  : 'SPERTE_printer_RC/Printer/Supervisory Controller/Homing feedforward/Discrete Derivative'
 * '<S19>'  : 'SPERTE_printer_RC/Reference/Compare To Constant'
 * '<S20>'  : 'SPERTE_printer_RC/Reference/Compare To Constant1'
 * '<S21>'  : 'SPERTE_printer_RC/Reference/Compare To Constant2'
 * '<S22>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System'
 * '<S23>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System1'
 * '<S24>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System/Input Delay'
 * '<S25>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System/Output Delay'
 * '<S26>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System1/Input Delay'
 * '<S27>'  : 'SPERTE_printer_RC/Repetitive Controller/LTI System1/Output Delay'
 */
#endif                                 /* SPERTE_printer_RC_h_ */
