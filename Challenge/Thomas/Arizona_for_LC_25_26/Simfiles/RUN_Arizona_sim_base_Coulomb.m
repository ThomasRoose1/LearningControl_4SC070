...
% 
%     % Order of input shaper and feedforward filters
%     na = 0;                          % Order input shaper Cy
%     nb = 3;                          % Order feedforward Cff
% 
%     % Coulomb basis settings
%     use_coulomb_basis = 1;
%     v_eps = 1e-4;                    % smoothing around zero velocity
% 
%     % Total number of parameters
%     % theta = [theta_y ; theta_ff ; theta_coulomb]
...
%     end
% 
%     % Store theta for every trial
%     theta = zeros(ntheta, N_trials+1);
% 
...
%     We    = we   * eye(Nref);    We_sq    = sqrt(We);
%     Wf    = wf   * eye(Nref);    Wf_sq    = sqrt(Wf);
%     Wdf   = wdf  * eye(Nref);    Wdf_sq   = sqrt(Wdf);
...
%     history.r_y = zeros(N_trials, Nref, no);
% 
%     % With input shaper off, r_y = r
