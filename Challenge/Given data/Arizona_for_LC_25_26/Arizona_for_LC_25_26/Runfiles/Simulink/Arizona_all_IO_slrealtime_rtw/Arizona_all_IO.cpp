...
void Arizona_all_IO_step(void)
{
  ...
  temp = 6.2831853071795862 * Arizona_all_IO_M->Timing.t[0];

  /* SignalGenerator: '<Root>/Signal Generator' */
  Arizona_all_IO_B.SignalGenerator = std::sin
    (Arizona_all_IO_cal->SignalGenerator_Frequency * temp) *
    Arizona_all_IO_cal->SignalGenerator_Amplitude;

  /* Gain: '<Root>/Gain1' */
  Arizona_all_IO_B.Gain1 = Arizona_all_IO_cal->Gain1_Gain *
    Arizona_all_IO_B.SignalGenerator;

  /* Gain: '<Root>/Gain5' */
  Arizona_all_IO_B.Gain5 = Arizona_all_IO_cal->Gain5_Gain *
    Arizona_all_IO_B.SignalGenerator;

  /* Gain: '<Root>/Gain2' */
  Arizona_all_IO_B.Gain2 = Arizona_all_IO_cal->Gain2_Gain *
    Arizona_all_IO_B.Gain5;
...

  {
     ...
     */
    if (!(++Arizona_all_IO_M->Timing.clockTick1)) {
      ++Arizona_all_IO_M->Timing.clockTickH1;
    }

    Arizona_all_IO_M->Timing.t[1] = Arizona_all_IO_M->Timing.clockTick1 *
      Arizona_all_IO_M->Timing.stepSize1 + Arizona_all_IO_M->Timing.clockTickH1 *
      Arizona_all_IO_M->Timing.stepSize1 * 4294967296.0;
  }
}
...
void Arizona_all_IO_initialize(void)
{
  ...
  {
    /* Setup solver object */
    rtsiSetSimTimeStepPtr(&Arizona_all_IO_M->solverInfo,
                          &Arizona_all_IO_M->Timing.simTimeStep);
    rtsiSetTPtr(&Arizona_all_IO_M->solverInfo, &rtmGetTPtr(Arizona_all_IO_M));
    rtsiSetStepSizePtr(&Arizona_all_IO_M->solverInfo,
                       &Arizona_all_IO_M->Timing.stepSize0);
    rtsiSetErrorStatusPtr(&Arizona_all_IO_M->solverInfo, (&rtmGetErrorStatus
      (Arizona_all_IO_M)));
    rtsiSetRTModelPtr(&Arizona_all_IO_M->solverInfo, Arizona_all_IO_M);
  }

  rtsiSetSimTimeStep(&Arizona_all_IO_M->solverInfo, MAJOR_TIME_STEP);
  ...
  /* Initialize timing info */
  {
    ...
    /* task periods */
    Arizona_all_IO_M->Timing.sampleTimes[0] = (0.0);
    Arizona_all_IO_M->Timing.sampleTimes[1] = (0.001);

    /* task offsets */
    Arizona_all_IO_M->Timing.offsetTimes[0] = (0.0);
    Arizona_all_IO_M->Timing.offsetTimes[1] = (0.0);
  }
  ...
  Arizona_all_IO_M->Timing.stepSize1 = 0.001;
  Arizona_all_IO_M->solverInfoPtr = (&Arizona_all_IO_M->solverInfo);
...

  {
    ...
    Arizona_all_IO_B.SignalGenerator = 0.0;
    Arizona_all_IO_B.Gain1 = 0.0;
    Arizona_all_IO_B.Gain5 = 0.0;
  ...
  /* child S-Function registration */
  {
    ...
    Arizona_all_IO_M->Sizes.numSampTimes = (2);
    ...
    Arizona_all_IO_M->NonInlinedSFcns.taskTimePtrs[0] = &(rtmGetTPtr
      (Arizona_all_IO_M)[0]);
    Arizona_all_IO_M->NonInlinedSFcns.taskTimePtrs[1] = &(rtmGetTPtr
      (Arizona_all_IO_M)[1]);
