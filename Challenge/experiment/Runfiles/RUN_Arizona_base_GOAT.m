...
%     history.r_y = zeros(N_trial,Nref,no);     % initialize shaped reference
    ...
    % Interconnection.
    [S1,PS1] = ClosedLoopTransfers(P_zpk,C_zpk);
    ...
    history.r_y = history.r; %zeros(N_trial,Nref,no);    
end
...
for trial = 0:N_trial-1
    ...
    if abs(f_appl) > f_threshold
        waitfor(msgbox('New feedforward should not be loaded: out of bounds ('+string(f_threshold)+'V)'));
        break % makes the zero input feedforward the last trial

    elseif max(isnan(f_appl))
        waitfor(msgbox('New feedforward should not be loaded: NaN! Ending operation'));
        break;
...
%     end
    ...
    % Store trial data
    % Store position and error corresponding to reference and history.rffw
    history.p(jj,:,:)       = p_j;
    history.p2(jj,:,:)      = p2_j;
    history.e(jj,:,:)       = e_j;
