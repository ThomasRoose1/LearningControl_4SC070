...
% SP_lifted = liftedMIMOMatrix(PS,N);
% 
% 
% Phi(:,1:sum(na_vec)) = -S_lifted*Psi_y_r;
% Phi(:,sum(na_vec)+1:end) = SP_lifted*Psi_ff_r;
% 
% %% Regression matrix
% 
% X = [ ...
...
function [theta_delta, Phi, Psi_y_r, Psi_ff_r] = FeedforwardUpdate_ILC_BFIS_MIMOv2( ...
    na_vec, nb_vec, Psi_blocks, N, S, PS, ...
    We_sq, Wry_sq, Wdry_sq, Wf_sq, Wdf_sq, ...
    e_y, r_y, f, r, t, Ts, ...
    ~, ~) % Ignored coulomb variables for now

    % 1. Unpack Parameters for clarity
    na_x = na_vec(1);   na_phi = na_vec(2);
    nb_x = nb_vec(1);   nb_phi = nb_vec(2);
    Psi_x = Psi_blocks{1}; Psi_phi = Psi_blocks{2};

    % 2. Hardcode the reference to strictly use the X-channel trajectory
    r_base = r(:,1); 

    %% ====================================================================
    %  STEP 1: Evaluate Direct Basis Responses (Psi)
    % =====================================================================
    % --- X Channel ---
    if na_x > 0
        Psi_y_r_x = brfus_v003(Psi_x(1:na_x).', r_base, t, Ts);
    else
        Psi_y_r_x = zeros(N, 0);
    end
    
    if nb_x > 0
        Psi_ff_r_x = brfus_v003(Psi_x(na_x+1:end).', r_base, t, Ts);
    else
        Psi_ff_r_x = zeros(N, 0);
    end

    % --- Phi Channel ---
    if na_phi > 0
        Psi_y_r_phi = brfus_v003(Psi_phi(1:na_phi).', r_base, t, Ts);
    else
        Psi_y_r_phi = zeros(N, 0);
    end
    
    if nb_phi > 0
        Psi_ff_r_phi = brfus_v003(Psi_phi(na_phi+1:end).', r_base, t, Ts);
    else
        Psi_ff_r_phi = zeros(N, 0);
    end

    %% ====================================================================
    %  STEP 2: Evaluate Error Prediction Responses (Phi)
    % =====================================================================
    % Initialize Phi blocks (2N rows because S and PS output both X and Phi)
    Phi_IS_x   = zeros(2*N, na_x);
    Phi_FF_x   = zeros(2*N, nb_x);
    Phi_IS_phi = zeros(2*N, na_phi);
    Phi_FF_phi = zeros(2*N, nb_phi);

    % --- X Channel ---
    for k = 1:na_x
        resp = -brfus_v003(S(:,1) * Psi_x(k), r_base, t, Ts); % Outputs N x 2
        Phi_IS_x(:,k) = resp(:); % Flatten to 2N x 1 (X on top, Phi on bottom)
    end
    for k = 1:nb_x
        resp = brfus_v003(PS(:,1) * Psi_x(na_x+k), r_base, t, Ts); 
        Phi_FF_x(:,k) = resp(:);
    end

    % --- Phi Channel ---
    for k = 1:na_phi
        resp = -brfus_v003(S(:,2) * Psi_phi(k), r_base, t, Ts); 
        Phi_IS_phi(:,k) = resp(:);
    end
    for k = 1:nb_phi
        resp = brfus_v003(PS(:,2) * Psi_phi(na_phi+k), r_base, t, Ts); 
        Phi_FF_phi(:,k) = resp(:);
    end

    %% ====================================================================
    %  STEP 3: Construct the X Matrix natively in the correct order
    %  Order: [IS_x, FF_x, IS_phi, FF_phi]
    % =====================================================================
    
    % Block 1: Input Shaper X
    X_IS_x = [We_sq * Phi_IS_x;
             -Wry_sq * [Psi_y_r_x; zeros(N, na_x)];
             -Wdry_sq * [Psi_y_r_x; zeros(N, na_x)];
             zeros(2*N, na_x);
             zeros(2*N, na_x)];

    % Block 2: Feedforward X
    X_FF_x = [We_sq * Phi_FF_x;
             zeros(2*N, nb_x);
             zeros(2*N, nb_x);
             -Wf_sq * [Psi_ff_r_x; zeros(N, nb_x)];
             -Wdf_sq * [Psi_ff_r_x; zeros(N, nb_x)]];

    % Block 3: Input Shaper Phi
    X_IS_phi = [We_sq * Phi_IS_phi;
               -Wry_sq * [zeros(N, na_phi); Psi_y_r_phi];
               -Wdry_sq * [zeros(N, na_phi); Psi_y_r_phi];
               zeros(2*N, na_phi);
               zeros(2*N, na_phi)];

    % Block 4: Feedforward Phi
    X_FF_phi = [We_sq * Phi_FF_phi;
               zeros(2*N, nb_phi);
               zeros(2*N, nb_phi);
               -Wf_sq * [zeros(N, nb_phi); Psi_ff_r_phi];
               -Wdf_sq * [zeros(N, nb_phi); Psi_ff_r_phi]];

    % Combine blocks side-by-side
    X = [X_IS_x, X_FF_x, X_IS_phi, X_FF_phi];

    %% ====================================================================
    %  STEP 4: Construct Y Vector and Solve
    % =====================================================================
    Y = [We_sq * e_y(:);
         Wry_sq * r_y(:);
         zeros(2*N, 1);
    ...
    % Because we built X block-by-block, theta is naturally in the exact 
    % order the main script expects!
    theta_delta = th_scaled ./ colNorms.';

    %% ====================================================================
    %  STEP 5: Format Outputs for Main Script
    % =====================================================================
    Phi      = [Phi_IS_x, Phi_FF_x, Phi_IS_phi, Phi_FF_phi];
    Psi_y_r  = [Psi_y_r_x, Psi_y_r_phi];
    Psi_ff_r = [Psi_ff_r_x, Psi_ff_r_phi];
